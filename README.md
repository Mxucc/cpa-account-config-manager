...
- 「总览」把 OpenCode 自己的用量放在最前面：总 Token（输入 / 输出 / 缓存拆分）、请求数，以及按 OpenCode 官方价格折算的**参考价**金额。金额只累计能证明属于本插件 OpenCode 渠道的运行时数据——Go 与 Zen 凭据按账号 ID 或工作区 ID 归因，无法归属的流量不会被算进来，也不会因为归属不了就显示成 0。下方按 5 小时 / 7 天 / 30 天给出额度窗口，每个窗口取用量最高的那个账号并给出重置倒计时；完全没有记录到 OpenCode 流量时，这里如实显示「尚未记录到 OpenCode 供应商流量」，而不是一排 0。
