...
```bash
sha256sum -c cpa-account-config-manager_0.2.9_linux_amd64.zip.sha256
```

macOS verification:

```bash
shasum -a 256 -c cpa-account-config-manager_0.2.9_darwin_arm64.zip.sha256
```

Windows PowerShell verification:

```powershell
Get-FileHash .\cpa-account-config-manager_0.2.9_windows_amd64.zip -Algorithm SHA256
Get-Content .\cpa-account-config-manager_0.2.9_windows_amd64.zip.sha256
```
...
```text
plugins/linux/amd64/cpa-account-config-manager-v0.2.9.so
plugins/linux/arm64/cpa-account-config-manager-v0.2.9.so
plugins/darwin/arm64/cpa-account-config-manager-v0.2.9.dylib
plugins/windows/amd64/cpa-account-config-manager-v0.2.9.dll
```
...
```bash
chmod 755 plugins/linux/amd64/cpa-account-config-manager-v0.2.9.so
```
...
services:
  cpa:
    volumes:
      - ./plugins/linux/amd64/cpa-account-config-manager-v0.2.9.so:/app/plugins/linux/amd64/cpa-account-config-manager-v0.2.9.so:ro
