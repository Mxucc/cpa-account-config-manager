...
- Codex 客户端身份策略在「Codex」页面中配置，支持出站身份收敛、官方客户端入口门、App Server 放行、最低/最高版本、白名单/黑名单、引擎指纹信号，以及关闭、设备级、会话级和完全收敛模式；原先位于「其他配置 → 实验性功能」的 Codex 实验项与身份策略编辑器都已迁移到该页面。
...
- 原先位于「其他配置 → 实验性功能」中的 Codex 实验项（Codex 5h / 7d 额度透支续用、Agent Identity 和 Codex 身份策略编辑器）改由「总览」承载；实验性功能因此只保留非 Codex 选项，并在保存时把 Codex 名下的取值原样回显、不做修改。
...
- OpenCode Go 路由按会话派生会话 ID：OpenCode Go 要求客户端“Send a stable session ID in `x-opencode-session` for each conversation so we can optimize routing and prompt caching”（引自 https://opencode.ai/docs/go/#where-can-i-use-it）；请求归属到 OpenCode 后，插件按以下顺序取值：入站 `x-opencode-session` 原样保留；否则复用原生客户端会话请求头（可识别 Claude Code、Codex、ZCode、Pi 风格的请求头）；再取请求体中的会话 ID（`prompt_cache_key`、`session_id`、`conversation_id`）；最后回退为对会话前缀的加盐摘要，使同一会话在各轮次保持同一 ID，且任何消息正文都不会被发送。归属 OpenCode 有两条路径：CPA 为其记录了 OpenCode 渠道的鉴权索引时，以该索引为准；宿主没有上报索引（例如 API Key 渠道）时，改由模型判定——模型由 OpenCode 发布且请求不属于 Codex 流量，即归属 OpenCode。该模型回退是必要的：OpenCode Go 会拒绝未携带 `x-opencode-session` 的请求，渠道列表一时未知时不能因此不发该请求头。注入仅限 OpenCode 发布的模型 ID（Zen 与 Go 目录，以及各账号已加载的目录），不触碰其他模型。
