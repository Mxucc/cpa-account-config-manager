...
```bash
sha256sum -c cpa-account-config-manager_*_linux_amd64.zip.sha256
```

macOS：

```bash
shasum -a 256 -c cpa-account-config-manager_*_darwin_arm64.zip.sha256
```

Windows PowerShell：

```powershell
$archive = Get-ChildItem .\cpa-account-config-manager_*_windows_amd64.zip -File | Sort-Object LastWriteTime -Descending | Select-Object -First 1
...
```text
plugins/linux/amd64/cpa-account-config-manager-v<version>.so
plugins/linux/arm64/cpa-account-config-manager-v<version>.so
plugins/darwin/arm64/cpa-account-config-manager-v<version>.dylib
plugins/windows/amd64/cpa-account-config-manager-v<version>.dll
```

Linux/macOS 上确保 CLIProxyAPI 服务账号可读、可执行：

```bash
chmod 755 plugins/linux/amd64/cpa-account-config-manager-v*.so
```
...
每行的活动图标会打开模型测试弹窗，并按提供方预选模型。确认后，插件通过 CPA 鉴权后的 `/v0/management/api-call` 发送一次最小 `hi` 生成请求，用 `auth_index` 固定目标账号，因此 Token 刷新和账号代理选择仍由 CPA 负责。只读或运行时账号只要 CPA 能解析其运行时索引也可以测试；默认情况下测试不会修改账号配置或状态，只有下述由用户主动开启的自动模型白名单实验例外。

浏览器只能提交有长度和字符限制的账号 ID 与模型 ID，不能提交 URL、Header、Prompt、Payload、凭据或代理。插件只会构造 Codex/OpenAI、Claude、Gemini/AI Studio 和 xAI 的固定 HTTPS 请求，设置 20 秒超时和响应体上限；对外仅返回规范化可用性、固定原因码、提供方/模型 ID、延迟、时间，以及手动测试所需的有界脱敏诊断预览。不支持的提供方会直接返回结构化“暂不支持”，不会发起网络请求。

Codex ChatGPT 账号探测支持一次明确的 `gpt-5.6-sol -> gpt-5.5` 兼容回退。只有首轮返回已知的 HTTP 400 JSON 文案，明确说明 ChatGPT 账号不支持 `gpt-5.6-sol` 时才会回退；鉴权、额度、限流、超时、网络、普通 400、API Key 路径和实验探测都不会触发。结果会分别保留主模型、最终可用模型、总延迟和两次脱敏诊断响应，因此界面不会把“仅 5.5 可用”误显示成“5.6 可用”。

“其他配置 → 实验性功能 → Codex 自动模型白名单”默认关闭，并独立于巡检策略持久化。开启后，只有先出现已知的 5.6 不兼容响应、5.5 回退成功，插件才会额外发起一次最小 `gpt-5.4-mini` 兼容性探测。仅当 5.5 与 5.4-mini 都真实成功、CPA 返回非空账号模型目录，并且账号没有现存的人工白名单或黑名单时，插件才会写入只允许这两个模型的 `allow_only` 策略。超时、普通 429、鉴权错误或失败探测不会推断白名单；应用、跳过和失败都会写入操作日志。关闭实验时不会进行第三次探测，也不会自动写模型策略。

手动测试只返回有界、经过脱敏和字段白名单处理的诊断预览；模型输出、凭据和敏感响应字段不会返回、持久化或写入日志。每次结果都会作为 `model_test` 写入统一操作日志，只记录公开账号、最终模型 ID 与规范化原因。单个模型失败只供操作员判断，不会触发账号级自动禁用或删除。没有明确额度窗口或 `usage_limit_reached` 证据的普通 HTTP 429 会按临时失败处理，不再等同于额度耗尽；账号级额度、凭据撤销和停用证据仍按巡检策略正常处置。
...
发现新版本后页面会显示提示。真正安装委托给 CPA 鉴权插件商店，由宿主负责 Registry 来源、平台匹配、归档限制、Checksum 校验和最终落盘。自动安装默认关闭，首次开启必须确认，并且只在已鉴权巡检页面打开期间运行。CPA 插件商店安装成功并返回 `restart_required: false` 时，由 CPA 热重载配置，刷新页面即可使用新版前端；只有宿主明确返回 `restart_required: true` 或报告已加载库被锁定时才需要重启 CPA。安装失败或文件被占用时仍可手动重试。插件不会自行下载、删除或替换自己的动态库，也不会保存浏览器中的 Management Key。

支持本机制的版本还会在 `data_dir/runtime-instances/` 写入脱敏心跳声明。同一个 CPA 进程和数据目录内，稳定语义版本最高的插件实例独占定时巡检、自动处置、外部通知、默认策略扫描，以及尚未结束的批处理和强制同步。替换版本先声明接管，旧实例停止，新实例经过一个短暂接管窗口后再开始；崩溃实例的声明会在有界超时后失效。声明只包含随机实例 ID、插件版本、进程 incarnation 哈希、进程作用域哈希和时间戳。声明存储不可用时，后台自动化会安全停用。跨版本心跳声明是唯一的运行时启动保护，不再使用持久化的首次重启门槛。

Linux 后台所有权作用域会纳入内核 boot ID 和 `/proc/self/stat` 的进程启动标识。当前 CPA 进程内的插件库还共享随机标识，用于区分当前进程声明和上个进程遗留声明；incarnation 哈希不同时，即使旧心跳尚未超过有界超时时间也不会参与竞争。旧版 v1-v3 bootstrap 文件会迁移为 `ready` 的 v4 记录，不再阻止当前 owner，因此 CPA 自更新即使保留 PID 或环境身份也不会持续显示“等待首次重启”。同一进程里确实存在参与协议的替换实例时，新实例仍会先让旧实例停止，再启动后台自动化。

原生 ABI 还保证处理错误、可恢复 Panic 和响应分配失败时始终返回非空 JSON 信封。已被替代的实例会返回有界 `503`，不会继续提供账号数据，并在后台服务停止后主动归还不再使用的 Go 堆页。如果连续多次进程内更新后，CPA 日志出现 `decode plugin envelope management.handle: unexpected end of JSON input`，应安装包含本保护的版本并完整重启 CPA 一次；修改 Management 地址无法修复原生 ABI 空响应。
...
- 删除旧插件动态库不是运行时卸载操作。应通过 CPA 插件商店安装，让宿主执行配置热重载并明确返回是否确实需要重启进程。
...
services:
  cpa:
    volumes:
      - ./plugins/linux/amd64:/app/plugins/linux/amd64:ro
...
在精确的注解 `vX.Y.Z` tag 上执行 `make package` 时，包名和插件运行时版本会自动从 tag 派生。普通未打 tag 的提交执行 `make build` 时会注入明确的 `0.0.0-dev` 版本；只有本地打包确实需要自定义版本时才传入 `VERSION=<version>`。GitHub Release 工作流始终使用推送的发布 tag，这与 CPA 插件商店规范一致：实际安装版本来自 GitHub latest release tag，registry 中的版本只作为展示兜底。
