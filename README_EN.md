...
- OpenCode Go sessions are derived per conversation: OpenCode Go asks clients to “Send a stable session ID in `x-opencode-session` for each conversation so we can optimize routing and prompt caching” (from https://opencode.ai/docs/go/#where-can-i-use-it). Once a request is attributed to OpenCode, the plugin resolves that value in order: an inbound `x-opencode-session` is preserved; otherwise a native client session header is reused (Claude Code, Codex, ZCode, and Pi style headers are recognized); then a session id found in the request body (`prompt_cache_key`, `session_id`, `conversation_id`); and finally a keyed digest of the conversation prefix, so the same conversation keeps the same id across turns while no message text is ever sent. Attribution takes two paths: when CPA records an auth index for an OpenCode channel, that index is authoritative; when the host does not report one, as happens for API-key channels, the model decides — a model published by OpenCode on a request that is not Codex traffic is attributed to OpenCode. That model fallback is required because OpenCode Go rejects a request that carries no `x-opencode-session`, so a channel list that is momentarily unknown must not stop the header from being sent. Injection is limited to model ids published by OpenCode (the Zen and Go catalogs plus each account's loaded catalog); no other model is touched.
