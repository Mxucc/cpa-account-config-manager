...
async function responseErrorMessage(
  ...
  try {
    ...
    if (preferError && typeof body.error === "string" && body.error.trim())
      return body.error;
...
async function fetchWithTimeout(
  ...
  try {
    ...
    if (timedOut && !init.signal?.aborted)
      throw new APIError(504, "ui.request_timeout");
    throw error;
  } finally {
...
async function managementRequest<T>(
  ...
  if (init.body && !isFormData && !headers.has("Content-Type"))
    headers.set("Content-Type", "application/json");
  const response = await fetchWithTimeout(buildManagementURL(path), {
    ...init,
    headers,
  });
  if (!response.ok) {
...
function parseCPAServerVersion(value: string): ParsedCPAServerVersion | null {
  const match =
    /^v?(\d+)\.(\d+)\.(\d+)(?:-([0-9A-Za-z.-]+))?(?:\+[0-9A-Za-z.-]+)?$/i.exec(
      value.trim(),
    );
  ...
  if (
    core.some(
      (part) => !Number.isSafeInteger(part) || part < 0 || part > 1_000_000,
    )
  )
    return null;
...
export function compareCPAServerVersions(
  left: string,
  right: string,
): number | null {
...
export async function getCPAServerVersionStatus(
  signal?: AbortSignal,
): Promise<CPAServerVersionSnapshot> {
  const session = getSession();
  if (!session) throw new APIError(401, "ui.management_key_is_not_set");
  ...
  try {
    response = await fetch(buildManagementURL("/latest-version"), {
      headers,
  ...
  if (response.status === 401)
    throw new APIError(401, "ui.authentication_failed");

  const currentVersion = safeCPAVersionLabel(
    response.headers.get("X-CPA-Version") ||
      response.headers.get("X-Server-Version"),
  );
  const currentBuildDate = safeCPAHeaderText(
    response.headers.get("X-CPA-Build-Date") ||
      response.headers.get("X-Server-Build-Date"),
  );
  let latestVersion = "";
  if (response.ok) {
    try {
      ...
      latestVersion = safeCPAVersionLabel(
        payload["latest-version"] ?? payload.latest_version ?? payload.latest,
      );
    } catch {
      latestVersion = "";
    }
  ...
  const comparison =
    currentVersion && latestVersion
      ? compareCPAServerVersions(currentVersion, latestVersion)
      : null;
  ...
  return {
    ...
    release_url:
      latestVersion && parseCPAServerVersion(latestVersion)
...
function hasRequiredNonNegativeIntegers(
  ...
  return keys.every(
    (key) =>
      ...
      isFiniteNonNegativeInteger(source[key]),
  );
}

function isValidAccountConcurrencyAvailability(
  value: unknown,
): value is AccountConcurrencyAvailability {
  return (
    isRecord(value) &&
    typeof value.supported === "boolean" &&
    hasRequiredNonNegativeIntegers(value, [
      "host_schema_version",
      "required_schema_version",
    ]) &&
    (value.reason === undefined ||
      value.reason === "host_schema_v2_required") &&
    (value.storage_error === undefined ||
      typeof value.storage_error === "string")
  );
}

function isValidAccountConcurrencySummary(
  value: unknown,
): value is NonNullable<AccountEditableConfig["concurrency"]> {
  return (
    isRecord(value) &&
    typeof value.supported === "boolean" &&
...
function normalizeAccountListResponse(
  response: unknown,
  ...
  if (
    ...
    response.accounts.some((account) => !isRecord(account))
  ) {
  ...
  if (
    !hasRequiredNonNegativeIntegers(source, [
      "total",
      "page",
      "page_size",
      "pages",
  ...
  const page = Math.max(
    1,
    finiteNonNegativeInteger(source.page, Math.max(1, requestedPage), 10_000),
  );
  ...
  const accounts = (source.accounts as Account[]).map((account) => {
    if (
      account.concurrency !== undefined &&
      !isValidAccountConcurrencySummary(account.concurrency)
    ) {
      throw new APIError(502, "ui.invalid_accounts_response");
    }
    return {
...
export async function listAccounts(
  ...
  const response = await requestRecord<AccountListResponse>(
    "/accounts",
    { signal },
    query,
  );
...
export async function loadAccountConfig(
  ...
  const validWebsockets =
    source.websockets === null || typeof source.websockets === "boolean";
  const validHeaders =
    source.header_names === null ||
    (Array.isArray(source.header_names) &&
      source.header_names.every((item) => typeof item === "string"));
  const validModelPolicy =
    source.model_policy === null || isRecord(source.model_policy);
  if (
    typeof source.account_id !== "string" ||
    !source.account_id.trim() ||
  ...
  return {
    ...
    account_id:
      typeof source.account_id === "string" ? source.account_id : accountID,
    header_names: Array.isArray(source.header_names)
      ? source.header_names.filter(
...
export async function refreshAccountQuotaMetadata(
  accountID: string,
): Promise<QuotaMetadataResponse> {
  return requestRecord<QuotaMetadataResponse>(
    "/accounts/quota-metadata/refresh",
    {
      method: "POST",
      body: JSON.stringify({ account_id: accountID }),
    },
...
export async function useAccountActiveReset(
  accountID: string,
): Promise<QuotaMetadataResponse> {
  return requestRecord<QuotaMetadataResponse>(
    "/accounts/quota-metadata/reset",
    {
      method: "POST",
...
export async function testAccountModel(
  accountID: string,
  model: string,
  experimentalWeeklyOverdraft = false,
...
export async function refreshAccountToken(
  accountID: string,
): Promise<AccountTokenRefreshResult> {
...
export async function loadAccountModels(
  scope: TargetScope,
): Promise<AccountModelCatalogResponse> {
  ...
  if (
    models !== undefined &&
    models.some((model) => !isNonEmptyString(model.id))
  ) {
    throw new APIError(502, "ui.invalid_api_response");
  }
  if (
    ...
    (source.warnings !== undefined &&
      (!Array.isArray(source.warnings) ||
        source.warnings.some((item) => typeof item !== "string")))
  ) {
    throw new APIError(502, "ui.invalid_api_response");
  }
  return {
...
export async function createAccountDeletePreview(
  accountID: string,
...
export async function createBatchDeletePreview(
  scope: TargetScope,
): Promise<BatchPreview> {
...
export async function scanAccountDuplicates(
  options: AccountDeduplicationOptions,
): Promise<AccountDeduplicationPreview> {
  return requestRecord<AccountDeduplicationPreview>(
    "/accounts/deduplicate/preview",
    {
      method: "POST",
...
export async function startBatchDelete(
  previewID: string,
): Promise<JobSnapshot> {
...
async function persistPluginSettings(
  settings: PersistentPluginSettings,
): Promise<void> {
  try {
    ...
    if (error instanceof APIError)
      throw new APIError(error.status, "ui.settings_persistence_failed");
    throw error;
  }
}

export async function saveDefaultPolicy(
...
const INSPECTION_ACTION_VALUES = new Set([
  "disable",
  "enable",
  "delete",
  "delete_candidate",
...
const INSPECTION_ACTION_STATUS_VALUES = new Set([
  "pending",
  "succeeded",
  "failed",
  "skipped",
...
function optionalNonNegativeInteger(
  ...
  return (
    ...
    item[key] === undefined ||
    item[key] === null ||
    isFiniteNonNegativeInteger(item[key])
  );
}
  ...
  return (
    ...
    (!("action" in item) ||
      (typeof item.action === "string" &&
        INSPECTION_ACTION_VALUES.has(item.action))) &&
    (!("status" in item) ||
      (typeof item.status === "string" &&
        INSPECTION_ACTION_STATUS_VALUES.has(item.status))) &&
    (!("reason_code" in item) || isNonEmptyString(item.reason_code)) &&
...
export async function getLiveInspection(
  signal?: AbortSignal,
): Promise<InspectionSnapshot> {
  ...
  if (
    !isRecord(source.policy) ||
    !isRecord(source.last_run) ||
...
export async function saveInspectionPolicy(
  ...
  confirmDeleteInvalidCredentials = false,
): Promise<InspectionSnapshot> {
  ...
  return requestRecord<InspectionSnapshot>("/inspection", {
    ...
    body: JSON.stringify({
      ...policy,
      confirm_auto_delete: confirmAutoDelete,
      confirm_delete_invalid_credentials: confirmDeleteInvalidCredentials,
    }),
  });
}

export async function scanFullInspection(): Promise<InspectionSnapshot> {
  return requestRecord<InspectionSnapshot>("/inspection/scan", {
    method: "POST",
  });
}

export async function scanNativeInspection(): Promise<InspectionSnapshot> {
  return requestRecord<InspectionSnapshot>("/inspection/scan/native", {
    method: "POST",
  });
}

export async function previewInspectionNotification(
  notification: InspectionNotificationRequest,
): Promise<InspectionNotificationPreview> {
  return requestRecord<InspectionNotificationPreview>(
    "/inspection/notification/preview",
    {
      method: "POST",
...
export async function testInspectionNotification(
  notification: InspectionNotificationRequest,
): Promise<InspectionNotificationTestResult> {
  return requestRecord<InspectionNotificationTestResult>(
    "/inspection/notification/test",
    {
      method: "POST",
      body: JSON.stringify(notification),
    },
...
export async function stopInspectionRun(): Promise<InspectionSnapshot> {
  return requestRecord<InspectionSnapshot>("/inspection/stop", {
    method: "POST",
  });
}
...
export async function listInspectionResults(
  ...
  const response = await requestRecord<unknown>(
    "/inspection/results",
    { signal },
    query,
  );
  ...
  if (
    results === undefined ||
    results.some((item) => !isValidInspectionResultRecord(item)) ||
  ...
  return {
    ...
    summary: {
      ...
      suggested_delete: finiteNonNegativeInteger(
        summary.suggested_delete,
        0,
        10_000_000,
      ),
      suggested_disable: finiteNonNegativeInteger(
        summary.suggested_disable,
        0,
        10_000_000,
      ),
      suggested_enable: finiteNonNegativeInteger(
        summary.suggested_enable,
        0,
        10_000_000,
      ),
      reauth: finiteNonNegativeInteger(summary.reauth, 0, 10_000_000),
      deletable_reauth: finiteNonNegativeInteger(
        summary.deletable_reauth,
        0,
        10_000_000,
      ),
      ...
      editable_enabled: finiteNonNegativeInteger(
        summary.editable_enabled,
        0,
        10_000_000,
      ),
      editable_disabled: finiteNonNegativeInteger(
        summary.editable_disabled,
        0,
        10_000_000,
      ),
    },
    ...
    page_size: Math.max(
      1,
      finiteNonNegativeInteger(source.page_size, pageSize, 1000),
    ),
    pages: Math.max(1, finiteNonNegativeInteger(source.pages, 1, 10_000_000)),
  } as InspectionResultList;
}

export async function listInspectionActions(
  limit = 50,
  signal?: AbortSignal,
): Promise<InspectionAction[]> {
  ...
  if (
    actions === undefined ||
    actions.some((item) => !isValidInspectionActionRecord(item))
  )
    throw new APIError(502, "ui.invalid_api_response");
  return actions as unknown as InspectionAction[];
}

export async function deleteInspectionRecommendations(
  accountIDs: string[],
): Promise<InspectionDeleteRun> {
...
export async function downloadInspectionExport(
  format: "json" | "csv" | "jsonl",
  health = "",
  search = "",
): Promise<{ filename: string; exported?: number }> {
  const session = getSession();
  if (!session) throw new APIError(401, "ui.management_key_is_not_set");
  ...
  const response = await fetchWithTimeout(
    buildURL("/inspection/export", query),
    {
  ...
  return {
    filename,
    exported: numericHeader(
      response.headers.get("X-Exported-Inspection-Results"),
    ),
  };
}

export async function executeInspectionAutoDelete(): Promise<InspectionDeleteRun> {
  return requestRecord<InspectionDeleteRun>("/inspection/auto-delete", {
    method: "POST",
  });
}
...
export async function getExperimentalSettings(
  signal?: AbortSignal,
): Promise<ExperimentalSettingsSnapshot> {
...
export async function getUsageLimits(
  ...
  return requestRecord<UsageLimitsSnapshot>("/usage-limits", { signal });
}

export async function saveUsageLimits(
  config: UsageLimitsConfig,
): Promise<UsageLimitsSnapshot> {
  return requestRecord<UsageLimitsSnapshot>("/usage-limits", {
    method: "PUT",
    body: JSON.stringify(config),
  });
}

export async function completeAgentIdentitySessionLogin(
  state: string,
  sessionJSON: string,
): Promise<AgentIdentitySessionLoginResponse> {
  return requestRecord<AgentIdentitySessionLoginResponse>(
    "/experiments/agent-identity/session-login",
    {
      method: "POST",
...
function normalizeOpenCodeAccountsResponse(
  response: unknown,
): OpenCodeAccountsResponse {
  ...
  if (
    ...
    accounts.some(
      (account) =>
        ...
        !isNonEmptyString(account.workspace_id),
    )
  ) {
    throw new APIError(502, "ui.invalid_api_response");
  }
  if (
    response.storage_error !== undefined &&
    typeof response.storage_error !== "string"
  ) {
    throw new APIError(502, "ui.invalid_api_response");
  }
  return {
    ...
    ...(typeof response.storage_error === "string"
      ? { storage_error: response.storage_error }
      : {}),
  };
}
...
export async function listOpenCodeAccounts(
  signal?: AbortSignal,
): Promise<OpenCodeAccountsResponse> {
  return normalizeOpenCodeAccountsResponse(
    await requestRecord<unknown>("/opencode/accounts", { signal }),
  );
}

export async function saveOpenCodeAccount(
  workspaceID: string,
  authCookie: string,
): Promise<OpenCodeAccountSaveResponse> {
...
export async function removeOpenCodeAccount(accountID: string): Promise<void> {
  await request<{ removed: boolean }>(
    "/opencode/accounts?account_id=" + encodeURIComponent(accountID),
    {
      method: "DELETE",
    },
  );
...
export async function refreshOpenCodeQuota(): Promise<{
  results: Record<string, import("../types").OpenCodeQuotaResult>;
}> {
...
export async function probeOpenCodeQuota(
  workspaceID: string,
  authCookie: string,
  timeoutSeconds = 30,
): Promise<OpenCodeProbeResponse> {
...
function normalizeOpenCodeZenAccountsResponse(
  response: unknown,
): OpenCodeZenAccountsResponse {
  if (!isRecord(response)) throw new APIError(502, "ui.invalid_api_response");
  const accounts = nullableRecordArray(response.accounts);
...
export async function listOpenCodeZenAccounts(
  signal?: AbortSignal,
): Promise<OpenCodeZenAccountsResponse> {
  return normalizeOpenCodeZenAccountsResponse(
    await requestRecord<unknown>("/opencode/zen/accounts", { signal }),
  );
}
...
export async function saveOpenCodeZenAccount(options: {
  ...
  return requestRecord<OpenCodeZenAccountSaveResponse>(
...
export async function removeOpenCodeZenAccount(
  accountID: string,
): Promise<void> {
  await request<{ removed: boolean }>(
    "/opencode/zen/accounts?account_id=" + encodeURIComponent(accountID),
    {
      method: "DELETE",
    },
  );
...
export async function probeOpenCodeZenAccount(
  accountID: string,
): Promise<OpenCodeZenProbeAccountResponse> {
  return requestRecord<OpenCodeZenProbeAccountResponse>(
    "/opencode/zen/probe-account?account_id=" + encodeURIComponent(accountID),
    {
      method: "POST",
    },
  );
...
export async function checkForUpdates(
  ...
  return requestRecord<UpdateSnapshot>("/updates/check", {
    method: "POST",
    signal,
  });
}
...
export async function getPluginStore(
  ...
  const response = await managementRequest<unknown>("/plugin-store", {
    signal,
  });
  ...
  return {
    ...
    plugins: (pluginValues ?? []) as unknown as NonNullable<
      PluginStoreResponse["plugins"]
    >,
  };
}

const pluginID = "cpa-account-config-manager";
const pluginReleaseBaseURL =
  "https://github.com/Mxucc/cpa-account-config-manager/releases/tag/v";

function normalizedStableVersion(
  value: string | undefined,
): { value: string; parts: [number, number, number] } | null {
...
export function reconcileUpdateStatus(
  ...
  const retainedError = obsoleteDirectCheckErrors.has(statusError)
    ? ""
    : statusError;
  const currentVersion = normalizedStableVersion(status.current_version);
  const plugin = store?.plugins_enabled
    ? arrayOrEmpty(store.plugins).find((entry) => entry?.id === pluginID)
    : undefined;
  ...
  const base: UpdateSnapshot = {
    ...
    store_error: storeError
      ? "plugin store metadata is unavailable"
      : undefined,
  ...
  const storeIsNewer =
    compareStableVersions(storeVersion.parts, currentVersion.parts) > 0;
...
async function loadPluginStore(
  signal?: AbortSignal,
): Promise<{ response: PluginStoreResponse | null; error: string }> {
...
export async function getEffectiveUpdateStatus(
  checkNow = false,
  signal?: AbortSignal,
): Promise<UpdateSnapshot> {
  ...
  void operation.then(
    () => {
      if (effectiveUpdateStatusInFlight[key] === operation)
        delete effectiveUpdateStatusInFlight[key];
    },
    () => {
      if (effectiveUpdateStatusInFlight[key] === operation)
        delete effectiveUpdateStatusInFlight[key];
...
const effectiveUpdateStatusInFlight: Partial<
  Record<"read" | "check", Promise<UpdateSnapshot>>
> = {};
...
async function getEffectiveUpdateStatusUnshared(
  checkNow: boolean,
  signal?: AbortSignal,
): Promise<UpdateSnapshot> {
  ...
  try {
    ...
    try {
      ...
      status = {
        ...
        pending: false,
        error:
          error instanceof Error ? error.message : "update status unavailable",
      };
    }
  }
...
export function installPluginUpdate(
  ...
  const install = installPluginUpdateOnce(version).finally(() => {
    if (pluginInstallInFlight.get(key) === install)
      pluginInstallInFlight.delete(key);
  });
  pluginInstallInFlight.set(key, install);
  return install;
}

async function installPluginUpdateOnce(
  version: string,
): Promise<PluginInstallResult> {
  try {
    ...
    if (
      !plugin ||
      !storeVersion ||
      compareStableVersions(storeVersion.parts, requestedVersion.parts) !== 0
    ) {
      throw new APIError(
        404,
    ...
    const installed = await managementRequest<PluginInstallResult>(
      "/plugin-store/cpa-account-config-manager/install",
      {
        method: "POST",
    ...
    if (
      ...
      !installedVersion ||
      compareStableVersions(installedVersion.parts, requestedVersion.parts) !==
        0
    ) {
    ...
    await recordBrowserOperation(
      "update_install",
      result.restart_required ? "warning" : "succeeded",
      result.version,
    ).catch(() => undefined);
    return result;
  } catch (error) {
    await recordBrowserOperation("update_install", "failed", version).catch(
      () => undefined,
    );
    throw error;
  }
...
export async function recordBrowserOperation(
...
export async function listOperations(
  ...
  try {
    response = await requestRecord<unknown>(
      "/operations",
      { signal: controller.signal },
      query,
    );
  ...
  if (
    !summaryKeys.every((key) => isFiniteNonNegativeInteger(summary[key])) ||
    !hasRequiredNonNegativeIntegers(source, [
      "total",
      "page",
      "page_size",
  ...
  if (
    source.extended_history !== undefined &&
    typeof source.extended_history !== "boolean"
  ) {
    throw new APIError(502, "ui.invalid_api_response");
  }
  for (const key of [
  ...
  const page = Math.max(
    1,
    finiteNonNegativeInteger(source.page, requestedPage, 10_000_000),
  );
  const pages = Math.max(
    1,
    finiteNonNegativeInteger(
      source.pages,
      total > 0 ? Math.ceil(total / 500) : 1,
      10_000_000,
    ),
  );
  return {
    ...
    archived_segments: finiteNonNegativeInteger(
      source.archived_segments,
      0,
      10_000_000,
    ),
    retention_limit: finiteNonNegativeInteger(
...
export async function saveOperationRetentionSettings(
  extendedHistory: boolean,
): Promise<OperationRetentionSettings> {
  await persistPluginSettings({
    operation_settings: { extended_history: extendedHistory },
  });
...
export async function getOperationRetentionSettings(): Promise<OperationRetentionSettings> {
  ...
  const hasDetails = ["page_size", "retained", "archived_segments"].some(
  ...
  if (!hasDetails) {
    ...
    return {
      extended_history: response.extended_history,
      page_size: 500,
      retained: 0,
      archived_segments: 0,
    };
  ...
  if (
    !isFiniteNonNegativeInteger(response.page_size) ||
...
export async function persistCurrentSettings(): Promise<ExperimentalSettings> {
  const [defaults, inspection, updates, operations, experiments] =
    await Promise.all([
      getDefaultPolicy(),
      getInspection(),
      getUpdateStatus(),
  ...
  await persistPluginSettings({
    ...
    operation_settings: {
      extended_history: operations.extended_history === true,
    },
...
const SETTINGS_MIGRATION_PREFIX =
  "cpa-account-config-manager:settings-migrated:";

function settingsMigrationKey(): string {
  const session = getSession();
  const base =
    session?.baseUrl ||
    (typeof window !== "undefined" ? window.location.origin : "default");
...
export async function clearOperations(): Promise<{
  operation: OperationEntry;
  retained: number;
}> {
  const response = await requestRecord<unknown>("/operations", {
    method: "DELETE",
  });
  ...
  if (
    !isRecord(response) ||
    !isRecord(response.operation) ||
    !isNonEmptyString(response.operation.id) ||
    typeof response.operation.status !== "string" ||
    !operationStatuses.has(response.operation.status) ||
    !isFiniteNonNegativeInteger(response.retained)
  ) {
    throw new APIError(502, "ui.invalid_api_response");
  }
...
export async function downloadOperationExport(
  ...
  if (filters.search) query.set("search", filters.search);
  const response = await fetchWithTimeout(
    buildURL("/operations/export", query),
    {
      headers: { Authorization: `Bearer ${session.managementKey}` },
    },
  );
  ...
  return {
    filename,
    exported: numericHeader(response.headers.get("X-Exported-Operations")),
  };
}

export async function createForceSyncPreview(): Promise<ForceSyncPreview> {
  return requestRecord<ForceSyncPreview>("/defaults/force/preview", {
    method: "POST",
  });
}

export async function startForceSync(
  previewID: string,
): Promise<ForceSyncJobSnapshot> {
...
export async function getForceSyncStatus(
  includeResults = true,
  signal?: AbortSignal,
): Promise<ForceSyncJobSnapshot> {
  const query = new URLSearchParams();
  if (!includeResults) query.set("light", "1");
  return requestRecord<ForceSyncJobSnapshot>(
    "/defaults/force/status",
    { signal },
    query,
  );
}
...
export async function getImportStatus(
  ...
  if (
    ...
    (source.usage_collection_started !== undefined &&
      typeof source.usage_collection_started !== "boolean") ||
    (source.usage_collection_targets !== undefined &&
      !isFiniteNonNegativeInteger(source.usage_collection_targets))
  ) {
    throw new APIError(502, "ui.invalid_api_response");
  }
  return {
...
export async function downloadExport(
  ...
  if (!session) throw new APIError(401, "ui.management_key_is_not_set");
  const query =
    kind === "accounts" && scope?.mode === "filtered"
      ? filtersQuery(scope.filters ?? {})
      : new URLSearchParams();
...
export const AI_PROVIDER_CHANNELS: Array<{
...
}> = [
  {
    ...
    labelKey: "ui.ai_provider_channel_openai_compatibility",
    apiPath: "/openai-compatibility",
  },
  {
    kind: "gemini-api-key",
    labelKey: "ui.ai_provider_channel_gemini",
    apiPath: "/gemini-api-key",
  },
  {
    kind: "interactions-api-key",
    labelKey: "ui.ai_provider_channel_interactions",
    apiPath: "/interactions-api-key",
  },
  {
    kind: "claude-api-key",
    labelKey: "ui.ai_provider_channel_claude",
    apiPath: "/claude-api-key",
  },
  {
    kind: "codex-api-key",
    labelKey: "ui.ai_provider_channel_codex",
    apiPath: "/codex-api-key",
  },
  {
    kind: "xai-api-key",
    labelKey: "ui.ai_provider_channel_xai",
    apiPath: "/xai-api-key",
  },
  {
    kind: "vertex-api-key",
    labelKey: "ui.ai_provider_channel_vertex",
    apiPath: "/vertex-api-key",
  },
  {
    kind: "api-keys",
    labelKey: "ui.ai_provider_channel_api_keys",
    apiPath: "/api-keys",
  },
  {
    kind: "opencode-go",
    labelKey: "ui.ai_provider_channel_opencode",
    apiPath: "",
  },
  {
    kind: "opencode-zen",
    labelKey: "ui.ai_provider_channel_opencode_zen",
    apiPath: "",
  },
];
...
function channelModelFromJSON(
  ...
  if (typeof source["display-name"] === "string")
    model.display_name = source["display-name"];
  if (typeof source["max-context-length"] === "number")
    model.max_context_length = source["max-context-length"];
  if (typeof source["force-mapping"] === "boolean")
    model.force_mapping = source["force-mapping"];
  if (typeof source["is-compat"] === "boolean")
    model.is_compat = source["is-compat"];
  if (typeof source["image"] === "boolean") model.image = source["image"];
  if (Array.isArray(source["input-modalities"]))
    model.input_modalities = source["input-modalities"].filter(
      (item): item is string => typeof item === "string",
    );
  if (Array.isArray(source["output-modalities"]))
    model.output_modalities = source["output-modalities"].filter(
      (item): item is string => typeof item === "string",
    );
  if (source["thinking"] !== undefined && source["thinking"] !== null)
    model.thinking = source["thinking"];
  return model;
}

function mapAPIKeyEntryFromJSON(
  source: Record<string, unknown>,
): AIProviderAPIKeyEntry {
  ...
  if (typeof source["auth-index"] === "string")
    entry.auth_index = source["auth-index"];
  else if (typeof source["auth_index"] === "string")
    entry.auth_index = source["auth_index"];
  ...
  if (typeof source["proxy-url"] === "string")
    entry.proxy_url = source["proxy-url"];
  return entry;
}

function channelEntriesFromResponse(
  kind: AIProviderChannelKind,
  payload: unknown,
): AIProviderChannelEntry[] {
  ...
  return raw.map((item, index) => {
    ...
    if (typeof source["api-key"] === "string")
      entry.api_key = source["api-key"];
    if (source["api-key-entries"] !== undefined) {
      if (
        !Array.isArray(source["api-key-entries"]) ||
        source["api-key-entries"].some((item) => !isRecord(item))
      ) {
        throw new APIError(502, "ui.invalid_api_response");
      }
      entry.api_key_entries = source["api-key-entries"].map((item) =>
        mapAPIKeyEntryFromJSON(item as Record<string, unknown>),
      );
      const firstKey = entry.api_key_entries.find(
        (keyEntry) => keyEntry.api_key,
      );
      if (firstKey?.api_key) entry.api_key = firstKey.api_key;
    }
    if (typeof source["base-url"] === "string")
      entry.base_url = source["base-url"];
    if (typeof source["proxy-url"] === "string")
      entry.proxy_url = source["proxy-url"];
    ...
    if (source["headers"] !== undefined) {
      if (
        ...
        Object.values(source["headers"]).some(
    ...
    if (source["models"] !== undefined) {
      if (
        ...
        source["models"].some((item) => !isRecord(item))
      ) {
        throw new APIError(502, "ui.invalid_api_response");
      }
      entry.models = source["models"].map((item) =>
        channelModelFromJSON(item as Record<string, unknown>),
      );
    }
    if (source["excluded-models"] !== undefined) {
      if (
        !Array.isArray(source["excluded-models"]) ||
        source["excluded-models"].some((item) => typeof item !== "string")
      ) {
        throw new APIError(502, "ui.invalid_api_response");
      }
      entry.excluded_models = source["excluded-models"] as string[];
    }
    if (typeof source["support-prompt-cache-key"] === "boolean")
      entry.support_prompt_cache_key = source["support-prompt-cache-key"];
    if (typeof source["disable-cooling"] === "boolean")
      entry.disable_cooling = source["disable-cooling"];
    if (source["request-retry"] !== undefined) {
      if (source["request-retry"] === null) entry.request_retry = null;
      else if (
        typeof source["request-retry"] === "number" &&
        Number.isInteger(source["request-retry"])
      )
        entry.request_retry = source["request-retry"];
      else throw new APIError(502, "ui.invalid_api_response");
    }
    if (source["request-scoped-errors"] !== undefined) {
      if (
        !Array.isArray(source["request-scoped-errors"]) ||
        source["request-scoped-errors"].some((item) => !isRecord(item))
      ) {
        throw new APIError(502, "ui.invalid_api_response");
      }
      // Keep the complete host rule objects so a later save preserves fields
      // this UI does not yet edit.
      entry.request_scoped_errors = source["request-scoped-errors"].map(
        (item) => ({ ...(item as Record<string, unknown>) }),
      );
    }
    if (typeof source["alpha-search"] === "boolean")
      entry.alpha_search = source["alpha-search"];
    if (typeof source["websockets"] === "boolean")
      entry.websockets = source["websockets"];
    if (typeof source["rebuild-mid-system-message"] === "boolean")
      entry.rebuild_mid_system_message = source["rebuild-mid-system-message"];
    if (typeof source["fingerprint-profile"] === "string")
      entry.fingerprint_profile = source["fingerprint-profile"];
    if (typeof source["auth-index"] === "string")
      entry.auth_index = source["auth-index"];
    return entry;
  });
 ...
 */
function normalizeAIProviderRuntimeResponse(
  response: unknown,
): AIProviderRuntimeResponse {
  if (
    !isRecord(response) ||
    !isNonEmptyString(response.updated_at) ||
  ...
  const snapshots = response.snapshots.map((raw) => {
    if (
      ...
      (raw.concurrency_configurable !== undefined &&
        typeof raw.concurrency_configurable !== "boolean") ||
...
export async function getAIProviderRuntime(
  signal?: AbortSignal,
): Promise<AIProviderRuntimeResponse> {
  return normalizeAIProviderRuntimeResponse(
    await managementRequest<unknown>("/ai-providers/runtime", { signal }),
  );
}
  ...
  try {
    if (channel.kind === "opencode-go") {
      ...
      return {
        ...
        ...(listed.storage_error
          ? { storage_error: "provider_storage_unavailable" }
          : {}),
      };
    }
    ...
    return {
      kind: channel.kind,
      count: 0,
      entries: [] as AIProviderChannelEntry[],
    };
  } catch (caught) {
    if (
      signal?.aborted ||
 ...
 */
export async function listAIProviderChannels(
  signal?: AbortSignal,
): Promise<AIProviderChannelSnapshot[]> {
  ...
  const remaining = await Promise.all(
    rest.map((channel) => loadAIProviderChannel(channel, signal)),
  );
  return [firstSnapshot, ...remaining];
}

export async function putAIProviderChannel(
  kind: AIProviderChannelKind,
  items: unknown[],
): Promise<void> {
...
async function getRawAIProviderChannelItems(
  kind: AIProviderChannelKind,
): Promise<unknown[]> {
...
export async function deleteAIProviderChannelEntry(
  kind: AIProviderChannelKind,
  index: number,
  accountID?: string,
): Promise<void> {
  ...
  await managementRequest<void>(`/${kind}?index=${index}`, {
    method: "DELETE",
  });
}

export async function patchAIProviderChannelEntry(
  kind: AIProviderChannelKind,
  index: number,
...
function keyEntriesToJSON(
  ...
  return entries.flatMap((entry) => {
    ...
    if (
      ...
      entry.weight !== null &&
      !(typeof entry.weight === "string" && entry.weight.trim() === "")
    ) {
    ...
    if (typeof entry.proxy_url === "string" && entry.proxy_url.trim())
      out["proxy-url"] = entry.proxy_url.trim();
...
export async function saveAIProviderChannelEntry(
  ...
  if (kind === "opencode-go" || kind === "opencode-zen") {
    throw new Error(
      "saveAIProviderChannelEntry only supports host-managed channels",
    );
  }
  const raw = await getRawAIProviderChannelItems(kind);
  if (index < 0 || index >= raw.length)
    throw new Error(kind + " entry #" + (index + 1) + " was not found");
  ...
  if (kind === "openai-compatibility") {
    ...
    const legacyAPIKey =
      typeof patched["api-key"] === "string" ? patched["api-key"].trim() : "";
    const originalHasKeyEntries = Array.isArray(
      (raw[index] as Record<string, unknown>)["api-key-entries"],
    );
    if (replacementAPIKey && originalHasKeyEntries) {
      ...
      if (keyEntries.length === 0)
        keyEntries.push({ "api-key": replacementAPIKey });
      else keyEntries[0] = { ...keyEntries[0], "api-key": replacementAPIKey };
    } else if (
      patch.api_key_entries === undefined &&
      keyEntries.length === 0 &&
      legacyAPIKey
    ) {
    ...
    if (
      patch.api_key_entries !== undefined ||
      replacementAPIKey ||
      (legacyAPIKey && !originalHasKeyEntries)
    ) {
      patched["api-key-entries"] = keyEntries;
    }
  ...
  if (patch.disabled !== undefined && kind === "openai-compatibility")
    patched["disabled"] = patch.disabled;
  if (patch.support_prompt_cache_key !== undefined)
    patched["support-prompt-cache-key"] = patch.support_prompt_cache_key;
  if (patch.disable_cooling !== undefined)
    patched["disable-cooling"] = patch.disable_cooling;
  ...
  if (patch.request_scoped_errors !== undefined) {
    if (patch.request_scoped_errors.length === 0)
      delete patched["request-scoped-errors"];
    else patched["request-scoped-errors"] = patch.request_scoped_errors;
  }
  if (patch.alpha_search !== undefined)
    patched["alpha-search"] = patch.alpha_search;
  if (patch.websockets !== undefined) patched["websockets"] = patch.websockets;
  if (patch.rebuild_mid_system_message !== undefined)
    patched["rebuild-mid-system-message"] = patch.rebuild_mid_system_message;
...
export async function testAIProviderChannel(
  baseURL: string,
  apiKey: string,
  timeoutSeconds = 15,
  headers?: Record<string, string>,
): Promise<AIProviderProbeResult> {
  return testAIProviderChannelForKind(
...
export async function testAIProviderChannelForKind(
  ...
  const response = await requestRecord<AIProviderProbeResult>(
    "/ai-providers/test",
    {
      method: "POST",
      body: JSON.stringify({
...
export async function setAIProviderChannelEnabled(
  kind: AIProviderChannelKind,
  index: number,
  enabled: boolean,
): Promise<void> {
  ...
  const current =
    typeof raw === "string"
      ? undefined
      : Number((raw as Record<string, unknown>)?.weight);
  const restoredWeight =
    typeof current === "number" && Number.isFinite(current) && current > 0
      ? current
      : 1;
  await patchAIProviderChannelEntry(kind, index, {
    weight: enabled ? restoredWeight : 0,
  });
}
...
export type NewAIProvider =
  | NewOpenAICompatibilityProvider
  | NewOpenCodeGoProvider
  | NewOpenCodeZenProvider
  | NewAPIKeyProvider;
...
export async function addAIProviderChannel(
  kind: AIProviderChannelKind,
  ...
  items.push({
    ...
    ...(apiKeyProvider.base_url?.trim()
      ? { "base-url": apiKeyProvider.base_url.trim() }
      : {}),
  });
  await putAIProviderChannel(kind, items);
}

function normalizeProxyProfilesResponse(
  response: unknown,
): ProxyProfileListResponse {
  if (!isRecord(response)) throw new APIError(502, "ui.invalid_api_response");
...
export async function listProxyProfiles(
  signal?: AbortSignal,
): Promise<ProxyProfileListResponse> {
  return normalizeProxyProfilesResponse(
    await requestRecord<unknown>("/proxy-profiles", { signal }),
  );
}

export async function createProxyProfile(
  input: ProxyProfileInput,
): Promise<ProxyProfileView> {
  const response = await requestRecord<{ profile?: unknown }>(
    "/proxy-profiles",
...
export async function updateProxyProfile(
  input: ProxyProfileInput & { id: string },
): Promise<ProxyProfileView> {
  const response = await requestRecord<{ profile?: unknown }>(
    "/proxy-profiles",
