        ...
        <div className="import-record-list">
          {visibleItems.map((item) => (
            <div className="import-record" key={`${item.index}:${item.target_name}`}>
              <div className="import-record-identity"><strong>{item.label}</strong><span>{item.account_id || item.email || `#${item.index}`}</span>{item.credential_type === "agent_identity" ? <b className="credential-type-tag">{tx("ui.agent_identity")}</b> : null}{item.credential_type === "personal_access_token" ? <b className="credential-type-tag">{tx("ui.codex_personal_access_token")}</b> : null}</div>
