...
export const ruUI: Record<UIMessageKey, string> = {
  ...
  "ui.codex_ingress_gate_description": "Глобальный переключатель является главным: когда он включен, учетные записи с codex_cli_only запрещаются до upstream по черному списку, белому списку, границам версий и отпечаткам движка; когда он выключен, плагин больше не отклоняет запросы даже при наличии этого флага (хост может ограничивать их сам). Явная политика учетной записи или провайдера переопределяет оба состояния.",
