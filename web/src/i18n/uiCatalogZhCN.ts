...
export const zhCNUI: Record<UIMessageKey, string> = {
  ...
  "ui.weekly_overdraft_automation_behavior": "七日额度耗尽后随巡检执行最多 5 次实验探测；任意一次成功则保持启用，5 次全部失败才自动禁用，并在自动启用开启时于七日额度重置后恢复。",
  "ui.availability_notice": "可用性",
  "ui.weekly_overdraft_availability_notice": "不保证一定能够继续生成，可透支额度也可能因订阅类型而不同。",
  "ui.weekly_overdraft_probe_gate": "周额度透支探测",
  ...
  "ui.notification_delivery_policy": "触发策略",
  "ui.notification_delivery_policy_number": "通知链接 {number} 的触发策略",
  "ui.generic_notification": "通用通知",
  "ui.policy_notifications": "策略通知",
  "ui.policy_notifications_description": "按供应商、套餐类型和邮箱后缀嵌套筛选账号，并在筛选集合低于可用阈值时通知。",
  "ui.add_notification_policy": "添加通知策略",
  ...
  "ui.confirm_remove_notification_policy": "确定删除通知策略 {name}？绑定的通知链接将恢复为通用通知。",
  ...
  "ui.notification_policy_binding_invalid": "有通知链接引用了已不存在的策略",
  "ui.generic_notification_endpoint_required": "启用通用通知触发前，至少启用一条通用通知链接",
  "ui.policy_notification_endpoint_required": "启用通知策略前，至少启用并绑定一条通知链接",
  "ui.policy_notification_endpoint_required_named": "请为通知策略 {name} 启用并绑定一条通知链接",
  "ui.notification_policy_order_hint": "排序仅用于管理展示；所有匹配策略都会独立判断和触发。",
  "ui.save_notification_policy_before_testing": "请先保存通知策略和链接绑定，再进行预览或测试",
  ...
  "ui.notification_parameter_policy_id": "通知策略 ID",
