...
export const zhCNUI: Record<UIMessageKey, string> = {
  ...
  "ui.codex_auto_model_whitelist_description": "在手动模型测试中识别受限的 ChatGPT 账号模型集合，并为该账号应用白名单。",
  ...
  "ui.auto_model_whitelist_safety_notice": "默认关闭。无论是否开启实验，单个模型失败都不会禁用整个账号。",
  ...
  "ui.notification_test_scenario": "通知测试场景",
  "ui.notification_scenario_manual": "手动测试",
  "ui.notification_scenario_anomaly": "异常占比",
  "ui.notification_scenario_available_accounts": "可用账号数",
  "ui.notification_scenario_availability_percent": "账号可用率",
  "ui.notification_scenario_combined": "组合场景",
  ...
  "ui.plugin_version_installed_refresh_to_use_the_new_version": "插件 {version} 已安装，刷新页面后使用新版本",
	 "ui.native_plugin_update_requires_a_full_cpa_restart": "原生插件更新后必须完整重启 CPA；仅刷新页面或删除旧文件不会停止内存中的旧实例",
	 "ui.runtime_ownership_storage_is_unavailable": "单实例运行状态无法持久化，后台自动化已安全停用，请检查 data_dir 写入权限",
	 "ui.restart_cpa_to_enable_this_plugin_instance_s_background_automation": "当前修复版正在等待首次 CPA 重启，重启前不会启动新的巡检、自动处置或外部通知",
