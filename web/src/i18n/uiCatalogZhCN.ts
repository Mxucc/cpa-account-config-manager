...
export const zhCNUI: Record<UIMessageKey, string> = {
  ...
  "ui.codex_ingress_gate_description": "全局开关是主开关：开启时，对标记 codex_cli_only 的账号在上游前用黑名单、白名单、版本范围和引擎指纹判定并拒绝；关闭后插件不再拦截任何账号，即使该账号仍带有该标记（上游可能仍会自行限制）。单个账号或供应商可在其策略中显式覆盖。",
