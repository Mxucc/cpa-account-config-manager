...
export const zhTWUI: Record<UIMessageKey, string> = {
  ...
  "ui.weekly_overdraft_automation_behavior": "七日額度耗盡後隨巡檢執行最多 5 次實驗探測；任一次成功則保持啟用，5 次全部失敗才自動停用，並在自動啟用開啟時於七日額度重設後恢復。",
  "ui.availability_notice": "可用性",
  "ui.weekly_overdraft_availability_notice": "不保證一定能繼續產生內容，可透支額度也可能因訂閱類型而不同。",
  "ui.weekly_overdraft_probe_gate": "每週額度透支探測",
  ...
  "ui.notification_delivery_policy": "觸發策略",
  "ui.notification_delivery_policy_number": "通知連結 {number} 的觸發策略",
  ...
  "ui.add_notification_policy": "新增通知策略",
  ...
  "ui.confirm_remove_notification_policy": "確定刪除通知策略 {name}？綁定的通知連結將恢復為通用通知。",
  ...
  "ui.notification_policy_binding_invalid": "有通知連結引用了已不存在的策略",
  "ui.generic_notification_endpoint_required": "啟用通用通知觸發前，至少啟用一條通用通知連結",
  "ui.policy_notification_endpoint_required": "啟用通知策略前，至少啟用並綁定一條通知連結",
  "ui.policy_notification_endpoint_required_named": "請為通知策略 {name} 啟用並綁定一條通知連結",
  "ui.notification_policy_order_hint": "排序僅用於管理顯示；所有符合的策略都會獨立判斷和觸發。",
  "ui.save_notification_policy_before_testing": "請先儲存通知策略和連結綁定，再進行預覽或測試",
